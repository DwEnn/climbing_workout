...
**모든 운동에는 근거 수준이 표시되어야 한다.** 근거 없는 추천은 싣지 않는다.
...
### 뱃지 색상
- ✅ 직접 검증 Tier 2: `var(--blue)` 배경
- ✅ 직접 검증 Tier 3: `#a855f7` 배경
- ✅ 직접 검증 Tier 4: `var(--text-dim)` 배경
- 🔶 직접 검증 Tier A (일반 운동과학): `#f4a261` 배경 — 클라이밍 특화 아님을 별도 명시
- 🔶 원칙 적용: `var(--yellow)` 배경 — 직접 검증과 시각적으로 명확히 구분
